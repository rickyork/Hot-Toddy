<?php
  class Mail_MIMEPart { /**
    ...
    * @access public
    */ public function __construct($body = '', $parameters = array()) { if (!defined('MAIL_MIMEPART_CRLF')) { define( 'MAIL_MIMEPART_CRLF', defined('MAIL_MIME_CRLF') ? MAIL_MIME_CRLF : "\r\n", true ); } foreach ($parameters as $key => $value) { switch ($key) { case 'content_type': { $headers['Content-Type'] = $value . (isset($charset) ? '; charset="' . $charset . '"' : ''); break; } case 'encoding': { $this->encoding = $value; $headers['Content-Transfer-Encoding'] = $value; break; } case 'cid': { $headers['Content-Id'] = '<' . $value . '>'; break; } case 'disposition': { $headers['Content-Disposition'] = $value . (isset($dfilename) ? '; filename="' . $dfilename . '"' : ''); break; } case 'dfilename': { if (isset($headers['Content-Disposition'])) { $headers['Content-Disposition'] .= '; filename="' . $value . '"'; } else { $dfilename = $value; } break; } case 'description': { $headers['Content-Description'] = $value; break; } case 'charset': { if (isset($headers['Content-Type'])) { $headers['Content-Type'] .= '; charset="' . $value . '"'; } else { $charset = $value; } break; } } }  if (!isset($headers['Content-Type'])) { $headers['Content-Type'] = 'text/plain'; }  if (!isset($this->encoding)) { $this->encoding = '7bit'; }  $this->encoded = array(); $this->headers = $headers; $this->body = $body; } /**
    ...
    */ public function addSubPart($body, $parameters) { $this->subparts[] = new Mail_MIMEPart($body, $parameters); return $this->subparts[count($this->subparts) - 1]; } /**
    * Returns encoded data based upon encoding passed to it
    *
    * @param $data     The data to encode.
    * @param $encoding The encoding type to use, 7bit, base64,
    *                  or quoted-printable.
    */ private function getEncodedData($data, $encoding) { switch ($encoding) { case '8bit': case '7bit': { return $data; } case 'quoted-printable': { return $this->quotedPrintableEncode($data); } case 'base64': { return rtrim( chunk_split( base64_encode($data), 76, MAIL_MIMEPART_CRLF ) ); } default: { return $data; } } } /**
    * Encodes data to quoted-printable standard.
    *
    * @param $input    The data to encode
    * @param $maximumLineLength Optional max line length. Should
    *                  not be more than 76 chars
    */ private function quotedPrintableEncode($input, $maximumLineLength = 76) { $lines = preg_split("/\r?\n/", $input); $eol = MAIL_MIMEPART_CRLF; $escape = '='; $output = ''; while (list(, $line) = each($lines)) { $lineLengh = strlen($line); $newline = ''; for ($lineCounter = 0; $lineCounter < $lineLengh; $lineCounter++) { $character = substr($line, $lineCounter, 1); $dec = ord($character); if (($dec == 32) && ($lineCounter == ($lineLengh - 1)))  { $character = '=20'; } else if ($dec == 9) {  } else if (($dec == 61) OR ($dec < 32 ) || ($dec > 126)) { $character = $escape.strtoupper(sprintf('%02s', dechex($dec))); } if ((strlen($newline) + strlen($character)) >= $maximumLineLength)  { $output .= $newline.$escape.$eol;  $newline = ''; } $newline .= $character; } $output .= $newline.$eol; } $output = substr($output, 0, -1 * strlen($eol));  return $output; } } ?>