...
class hMailIMAPLibrary extends hPlugin {

    private $error;
    public $mailbox;
    private $mailboxInfo = array();
    ...
    */
    public function connect($uri, $getInfo = true)
    {        
        $uri = parse_url($uri);
        ...
        {
            // Specify the port based on the protocol
            switch ($uri['scheme'])
            {
                case 'imap':
                case 'imap2':
                ...
                default:
                {
                    $this->warning('The protocol supplied is not supported.', __FILE__, __LINE__);
                }
            }
        }
        
        if ($uri['scheme'] == 'imap2biss' || $uri['scheme'] != 'imap2bis')
        {
            $secure = ('tls' == substr($uri['fragment'], 0, 3))? '' : '/ssl';
            $connection .= ('s' == (substr($uri['scheme'], -1)))? '/'.substr($uri['scheme'], 0, 4).$secure : '/'.$uri['scheme'];
        }
        else
        {
            $secure = '';
        ...
        {
            $connection .= '/'.$uri['fragment'];
        }

        $connection .= '}';

        $this->mailboxInfo['host'] = $connection;

        // Trim off the leading slash '/'
        if (!empty($uri['path']))
        {
            $this->mailboxInfo['folder'] = substr($uri['path'], 1, (strlen($uri['path']) - 1));
            $connection .= $this->mailboxInfo['folder'];
        }
 
        $this->mailboxInfo['user'] = urldecode($uri['user']);
 
        $this->mailbox = imap_open(
            $connection,
            urldecode($uri['user']),
            urldecode($uri['pass']),
            isset($this->option['open'])? $this->option['open'] : null
        );
 
        if (false === $this->mailbox)
        {
            $this->warning('Unable to build a connection to the specified mail server.', __FILE__, __LINE__);
            $return = false;
        }
        else
        {
    ...
    * @access public
    * @see imap_open
    * @tutorial http://www.deadmarshes.com/Mail_IMAP?content=Mail_IMAP/getMailboxInfo
    */
    public function getMailboxInfo($return = true)
    {
        ...
        return $return? $this->mailboxInfo : true;
    }
    ...
    */
    public function setOptions(array $options, $constant)
    {
        foreach ($options as $value)
        {
            if (!$this->option[$value] = constant($constant))
            {
                $this->warning('The constant: '.$constant.' is not defined!.', __FILE__, __LINE__);
            }
        }
    ...
    */
    public function close()
    {
        return imap_close(
            $this->mailbox, 
            isset($this->option['close'])? $this->option['close'] : null
        );
    }
    ...
    */
    private function declareParts(&$messageId, $subPart = null, $subPartId = null, $n = 0, $isSubPart = false, $skipPart = false, $lastWasSigned = false)
    {
        if (!is_array($subPart))
        {
            $this->structure[$messageId]['obj'] = imap_fetchstructure(
                $this->mailbox,
                $messageId, 
                isset($this->option['fetchstructure'])? $this->option['fetchstructure'] : null
            );
        }

        if (isset($this->structure[$messageId]['obj']->parts) || is_array($subPart))
        {
            if (!$isSubPart)
            {
                $parts = $this->structure[$messageId]['obj']->parts;
            }
            else
            {
                $parts = $subPart;
            ...
            for ($p = 0, $i = 1; $p < count($parts); $n++, $p++, $i++)
            {
                ...
                if (empty($parts[$p]->type))
                {
                    $ftype = $this->dataTypes[0].'/'.strtolower($parts[$p]->subtype);
                }
                else
                {
                    $ftype = $this->dataTypes[$parts[$p]->type].'/'.strtolower($parts[$p]->subtype);
                }

                $this_was_signed = ($ftype == 'multipart/signed');
                $skip_next = ($ftype == 'message/rfc822');
                
                $skip = (
                    $ftype == 'multipart/mixed' && ($lastWasSigned || $skipPart) ||
                    $ftype == 'multipart/signed' ||
                    $skipPart && $ftype == 'multipart/alternative' ||
                ...
                else
                {
                    ...
                    $this->structure[$messageId]['pid'][$n] = !$isSubPart? (string) "$i" : (string) "$subPartId.$i";
                    $this->structure[$messageId]['ftype'][$n] = $ftype;
                    $this->structure[$messageId]['encoding'][$n] = (empty($parts[$p]->encoding))? $this->_encodingTypes[0] : $this->_encodingTypes[$parts[$p]->encoding];
                    $this->structure[$messageId]['fsize'][$n] = (!isset($parts[$p]->bytes) || empty($parts[$p]->bytes))? 0 : $parts[$p]->bytes;
                    ...
                    if ($parts[$p]->ifparameters)
                    {
                        ...
                        {
                            $this->structure[$messageId][strtolower($param->attribute)][$n] = strtolower($param->value);
                        }
                    }
                    ...
                    {
                        $this->structure[$messageId]['disposition'][$n] = strtolower($parts[$p]->disposition);
                        ...
                        if ($parts[$p]->ifdparameters)
                        {
                            foreach ($parts[$p]->dparameters as $param)
                            {
                                ...
                                {
                                    $this->structure[$messageId]['fname'][$n] = $param->value;
                                    break;
                                }
                            }
                        }
                    ...
                    {
                        $this->structure[$messageId]['disposition'][$n] = 'inline';
                    }

                    if ($parts[$p]->ifid)
                    {
                        $this->structure[$messageId]['cid'][$n] = $parts[$p]->id;
                    }
                }
                ...
                if (isset($parts[$p]->parts) && is_array($parts[$p]->parts))
                {
                    if (!$skipped)
                    {
                        $this->structure[$messageId]['has_at'][$n] = true;
                    }

                    $n = $this->declareParts($messageId, $parts[$p]->parts, $this->structure[$messageId]['pid'][$n], $n, true, $skip_next, $this_was_signed);
                }
                else if (!$skipped)
                {
                    $this->structure[$messageId]['has_at'][$n] = false;
                }
            }
        ...
        else
        {
            ...
            $this->structure[$messageId]['pid'][0] = 1;
            
            if (empty($this->structure[$messageId]['obj']->type))
            {
                $this->structure[$messageId]['obj']->type = (int) 0;
            }
            
            if (isset($this->structure[$messageId]['obj']->subtype))
            {
                $this->structure[$messageId]['ftype'][0] = $this->dataTypes[$this->structure[$messageId]['obj']->type].'/'.strtolower($this->structure[$messageId]['obj']->subtype);
            }
            
            if (empty($this->structure[$messageId]['obj']->encoding))
            {
                $this->structure[$messageId]['obj']->encoding = (int) 0;
            }
            
            $this->structure[$messageId]['encoding'][0] = $this->_encodingTypes[$this->structure[$messageId]['obj']->encoding];
            
            if (isset($this->structure[$messageId]['obj']->bytes))
            {
                $this->structure[$messageId]['fsize'][0] = strtolower($this->structure[$messageId]['obj']->bytes);
            }
            
            $this->structure[$messageId]['disposition'][0] = 'inline';
            $this->structure[$messageId]['has_at'][0] = false;
            
            // Go through the parameters, if any
            if (isset($this->structure[$messageId]['obj']->ifparameters) && $this->structure[$messageId]['obj']->ifparameters)
            {
                foreach ($this->structure[$messageId]['obj']->parameters as $param)
                {
                    $this->structure[$messageId][strtolower($param->attribute)][0] = $param->value;
                }
            }
        }
        return;
    ...
    */
    private function checkIfParsed(&$messageId, $checkPid = true, $get_mime = 'text/html')
    {
        if (!isset($this->structure[$messageId]['pid']))
        {
            $this->declareParts($messageId);
        }
 
        if ($checkPid && !isset($this->msg[$messageId]['pid']))
        {
            $this->getDefaultPartId($messageId, $get_mime);
        }
        return;
    }
    ...
    * @since PHP 4.2.0
    */
    public function getParts(&$messageId, $partId = '0', $return = false, $arguments = array())
    {
        if (!isset($arguments['get_mime']))
        {
            $arguments['get_mime'] = 'text/html';
        }

        if (!isset($arguments['get_alternative']))
        {
            $arguments['get_alternative'] = true;
        }
        
        $this->checkIfParsed($messageId, true, $arguments['get_mime']);

        if ($partId === '0')
        {
            $partId = $this->msg[$messageId]['pid'];
        }

        if (count($this->structure[$messageId]['pid']) == 1 && !isset($this->structure[$messageId]['fallback'][0]))
        {
            return true;
        }
    
        // retrieve key for this part, so that the information may be accessed
        if (false !== ($i = array_search((string) $partId, $this->structure[$messageId]['pid'])))
        {
            if (isset($arguments['retrieve_all']) && $arguments['retrieve_all'])
            {
                $this->scanMultipart($messageId, $partId, $i, $arguments['get_mime'], 'add', 'none', 2, $arguments['get_alternative']);
            }
            else
            {
                if ($partId == $this->msg[$messageId]['pid'])
                {
                    $this->scanMultipart($messageId, $partId, $i, $arguments['get_mime'], 'add', 'top', 2, $arguments['get_alternative']);
                }
                else if ($this->structure[$messageId]['ftype'][$i] == 'message/rfc822')
                {
                    $this->scanMultipart($messageId, $partId, $i, $arguments['get_mime'], 'add', 'all', 1, $arguments['get_alternative']);
                }
            }
        }
        else
        {
            $this->warning('The pid, '.$partId.' is not valid.', __FILE__, __LINE__);
            return false;
        }
    
        return ($return)? $this->msg[$messageId] : true;
    }
    ...
    */
    private function scanMultipart(&$messageId, &$partId, &$i, $mime, $action = 'add', $lookFor = 'all', $partIdAdd = 1, $getAlternative = true)
    {
        ...
        $partIdLength = strlen($partId);
        $thisNesting = count(explode('.', $partId));
        
        foreach ($this->structure[$messageId]['pid'] as $p => $id)
        {
            ...
            $nesting = count(explode('.', $this->structure[$messageId]['pid'][$p]));
            
            switch ($lookFor)
            {
                case 'all':
                {
                    $condition = (($nesting == ($thisNesting + 1)) && $partId == substr($this->structure[$messageId]['pid'][$p], 0, $partIdLength));
                    break;
                }
                case 'multipart':
                {
                    $condition = (($nesting == ($thisNesting + 1)) && ($partId == substr($this->structure[$messageId]['pid'][$p], 0)));
                    break;
                }
                ...
                default:
                {
                    if ($this->isMultipart($messageId, 'related') || $this->isMultipart($messageId, 'mixed'))
                    {
                        $condition = (
                            !stristr($this->structure[$messageId]['pid'][$p], '.') || 
                            ($nesting == 2) && 
                            substr($this->msg[$messageId]['pid'], 0, 1) == substr($this->structure[$messageId]['pid'][$p], 0, 1)
                        );
                    }
                    else
                    {
                        $condition = (!stristr($this->structure[$messageId]['pid'][$p], '.'));
                    }
                }
            }
            ...
            if ($condition)
            {
                if ($this->structure[$messageId]['ftype'][$p] == 'multipart/alternative' || $this->structure[$messageId]['ftype'][$p] == 'multipart/mixed')
                {
                    foreach ($this->structure[$messageId]['pid'] as $mp => $mpid)
                    {
                        // Part must begin with last matching part id and be two levels in
                        $sub_nesting = count(explode('.', $this->structure[$messageId]['pid'][$p]));
                        
                        $getAlternative = (
                            $this->structure[$messageId]['ftype'][$mp] == $mime &&
                            $getAlternative &&
                            ($sub_nesting == ($thisNesting + $partIdAdd)) &&
                            ($partId == substr($this->structure[$messageId]['pid'][$mp], 0, strlen($this->structure[$messageId]['pid'][$p])))
                        );
                        ...
                        if ($getAlternative)
                        {
                            if ($action == 'add')
                            {
                                $this->addPart($in, $messageId, $mp, 'in');
                                break;
                            }
                            else if ($action == 'get' && !isset($this->structure[$messageId]['fname'][$mp]) && empty($this->structure[$messageId]['fname'][$mp]))
                            {
                                return $this->structure[$messageId]['pid'][$mp];
                            }
                        }
                        else if ($this->structure[$messageId]['ftype'][$mp] == 'multipart/alternative' && $action == 'get')
                        {
                            // Need to match this PId to next level in
                            $partId = (string) $this->structure[$messageId]['pid'][$mp];
                            $partIdLength = strlen($partId);
                            $thisNesting = count(explode('.', $partId));
                            $partIdAdd = 2;
                            continue;
                ...
                else if ($this->structure[$messageId]['disposition'][$p] == 'inline' && $this->structure[$messageId]['ftype'][$p] != 'multipart/related' && $this->structure[$messageId]['ftype'][$p] != 'multipart/mixed')
                {
                    if ($action == 'add' && $partId != $this->structure[$messageId]['pid'][$p] && (($this->structure[$messageId]['ftype'][$p] != $excludeMime) || ($this->structure[$messageId]['ftype'][$p] == $excludeMime && isset($this->structure[$messageId]['fname'][$p])) || (isset($this->structure[$messageId]['fallback'][0]))))
                    {
                        $this->addPart($in, $messageId, $p, 'in');
                    }
                    else if ($action == 'get' && $this->structure[$messageId]['ftype'][$p] == $mime && !isset($this->structure[$messageId]['fname'][$p]))
                    {
                        return $this->structure[$messageId]['pid'][$p];
                    }
                }
                else if ($action == 'add' && $this->structure[$messageId]['disposition'][$p] == 'attachment')
                {
                    $this->addPart($a, $messageId, $p, 'at');
                }
            }
        }
    ...
    */
    private function isMultipart($messageId, $subtype)
    {
        $return = $this->extractMIME($messageId, array('multipart/'.$subtype));
        return (!empty($return) && is_array($return) && count($return) >= 1);
    }
    ...
    */
    public function getRelatedParts(&$messageId, &$partId, $secureMime = array())
    {
        // Check to see if this part has already been parsed
        $this->checkIfParsed($messageId);
        ...
        {
            $this->warning('Argument $secureMime is not an array.', __FILE__, __LINE__);
            return false;
        }

        $related = array();
 
        if (isset($this->structure[$messageId]['pid']) && is_array($this->structure[$messageId]['pid']))
        {
            if (strlen($partId) > 1)
            {
                $nesting = count(explode('.', $partId));
                $compare = substr($partId, 0, -4);

                foreach ($this->structure[$messageId]['pid'] as $i => $rpid)
                {
                    ...
                    {
                        $this->getCIds($messageId, $i, $secureMime, $related);
                    }
                }
            }
            else if (strlen($partId) == 1)
            {
                ...
                foreach ($this->structure[$messageId]['pid'] as $i => $rpid)
                {
                    ...
                    // of its parent part.
                    if (count(explode('.', $rpid)) == 2 && substr($rpid, 0, 1) == $partId)
                    {
                        $this->getCIds($messageId, $i, $secureMime, $related);
                    }
                }
        ...
        {
            $this->warning('Message structure does not exist.', __FILE__, __LINE__);
        }
    ...
    */
    private function getCIds(&$messageId, &$i, &$secureMime, &$related)
    {
        if ((isset($this->structure[$messageId]['cid'][$i])) && (empty($secureMime) || is_array($secureMime) && in_array($this->structure[$messageId]['ftype'][$i], $secureMime)))
        {
            $related['cid'][]   = $this->structure[$messageId]['cid'][$i];
            $related['pid'][]   = $this->structure[$messageId]['pid'][$i];
            $related['ftype'][] = $this->structure[$messageId]['ftype'][$i];
        }
    }
    ...
    */
    public function unsetParts(&$messageId)
    {
        unset($this->msg[$messageId]);
        unset($this->structure[$messageId]);
        return;
    }
    ...
    */
    private function addPart(&$n, &$messageId, &$i, $part)
    {
        foreach ($this->fields as $field)
        {
            if (isset($this->structure[$messageId][$field][$i]) && !empty($this->structure[$messageId][$field][$i]))
            {
                $this->msg[$messageId][$part][$field][$n] = $this->structure[$messageId][$field][$i];
            }
        }
        $n++;
        return;
    ...
    */
    public function getRawMessage(&$messageId)
    {
        return imap_body(
            $this->mailbox,
            $messageId,
            isset($this->option['body'])? $this->option['body'] : null
        );
    }
    ...
    * @since PHP 4.2.0
    */
    public function getBody(&$messageId, $partId = '1', $get_mime = 'text/html', $attempt = 1)
    {
        if ($this->hMailIMAPGetBody('literal'))
        {
            return imap_fetchbody(
                $this->mailbox,
                $messageId,
                $partId, 
                isset($this->option['fetchbody'])? $this->option['fetchbody'] : null
            );
        }
    
        $this->checkIfParsed($messageId, true, $get_mime);

        if (false !== ($i = array_search((string) $partId, $this->structure[$messageId]['pid'])))
        {
            if ($this->hMailIMAPGetBody('literalDecode'))
            {
                $messageBody = imap_fetchbody(
                    $this->mailbox,
                    $messageId,
                    $partId,
                    isset($this->option['fetchbody'])? $this->option['fetchbody'] : null
                ...
                return $this->decodeMessage($messageBody, $this->structure[$messageId]['encoding'][$i]);
            }
            ...
            switch ($this->structure[$messageId]['ftype'][$i])
            {
                case 'message/rfc822':
                case 'multipart/related':
                case 'multipart/alternative':
                {
                    if ($this->structure[$messageId]['ftype'][$i] == 'message/rfc822' || $this->structure[$messageId]['ftype'][$i] == 'multipart/related')
                    {
                        $new_pid = $this->scanMultipart($messageId, $partId, $i, $get_mime, 'get', 'all', 1);
                    }
                    else
                    {
                        $new_pid = $this->scanMultipart($messageId, $partId, $i, $get_mime, 'get', 'multipart', 1);
                    }
                    ...
                    switch(true)
                    {
                        ...
                        {
                            return ($attempt == 1)? $this->getBody($messageId, $partId, $action, 'text/plain', 2) : false;
                        }
                        case (empty($new_pid) && $get_mime == 'text/plain'):
                        {
                            return ($attempt == 1)? $this->getBody($messageId, $partId, $action, 'text/html', 2) : false;
                        }
                    }
                }
            }
            ...
            if (!empty($new_pid) && false === ($i = array_search((string) $partId, $this->structure[$messageId]['pid'])))
            {
                // Something's afoot!
                $this->warning('Unable to find a suitable replacement part Id. Message: may be poorly formed, corrupted, or not supported by the Mail_IMAPv2 parser.', __FILE__, __LINE__);
                return false;
            }
            ...
            if (!$messageBody)
            {
                $this->warning('Message body is null.', __FILE__, __LINE__);
                return false;
            }
            ...
            return array(
                'message' => $this->decodeMessage(
                    $messageBody,
                    $this->structure[$messageId]['encoding'][$i],
                    $this->structure[$messageId]['charset'][$i]
                ),
                'ftype' => $this->structure[$messageId]['ftype'][$i],
                'fname' => isset($this->structure[$messageId]['fname'][$i])? $this->structure[$messageId]['fname'][$i] : '',
                'charset' => $this->structure[$messageId]['charset'][$i]
            );
        }
        else
        {
            $this->warning('Invalid pid, '.$partId, __FILE__, __LINE__);
            return false;
        }
    ...
    public function getCharsetAndEncoding($messageId, $partId)
    {    
        if (false !== ($offset = array_search((string) $partId, $this->structure[$messageId]['pid'], true)))
        {
            return array(
                'charset' => $this->structure[$messageId]['charset'][$offset],
                'encoding' => $this->structure[$messageId]['encoding'][$offset]
            );
        }
    ...
    */
    private function decodeMessage(&$body, &$encoding, &$charset)
    {
        switch ($encoding)
        {
    ...
    * @access private
    */
    private function getDefaultPartId(&$messageId, $get_mime = 'text/html', $attempt = 1)
    {
        // Check to see if this part has already been parsed
        $this->checkIfParsed($messageId, false);

        // Look for a text/html message part
        // If no text/html message part was found look for a text/plain message part
        $part = array('text/html', 'text/plain');

        if ($get_mime == 'text/plain')
        {
        ...
        {
            if (0 !== count($msg_part = @array_keys($this->structure[$messageId]['ftype'], $mime)))
            {
                foreach ($msg_part as $i)
                {
                    if ($this->structure[$messageId]['disposition'][$i] == 'inline' && !stristr($this->structure[$messageId]['pid'][$i], '.'))
                    {
                        $this->msg[$messageId]['pid'] = $this->structure[$messageId]['pid'][$i];
                        return $this->structure[$messageId]['pid'][$i];
                    }
                }
            }
        }
        ...
        if (is_array($this->structure[$messageId]['pid']))
        {
            foreach ($this->structure[$messageId]['pid'] as $p => $id)
            {
                $nesting = count(explode('.', $this->structure[$messageId]['pid'][$p]));
                ...
                if (!isset($mpid))
                {
                    if ($nesting == 1 && isset($this->structure[$messageId]['ftype'][$p]) && ($this->structure[$messageId]['ftype'][$p] == 'multipart/related'))
                    {
                        $mp_nesting = 2;
                        $partIdLength = 3;
                        continue;
                    ...
                    if ($nesting == $mp_nesting && isset($this->structure[$messageId]['ftype'][$p]) && ($this->structure[$messageId]['ftype'][$p] == 'multipart/alternative' || $this->structure[$messageId]['ftype'][$p] == 'multipart/mixed'))
                    {
                        $mpid = $this->structure[$messageId]['pid'][$p];
                        continue;
                    }
                }

                if (isset($mpid) && $nesting == ($mp_nesting + 1) && $this->structure[$messageId]['ftype'][$p] == $get_mime && $mpid == substr($this->structure[$messageId]['pid'][$p], 0, $partIdLength))
                {
                    $this->msg[$messageId]['pid'] = $this->structure[$messageId]['pid'][$p];
                    return $this->structure[$messageId]['pid'][$p];
                }
            }
        }
        ...
        switch ($get_mime)
        {
            case 'text/html':
            {
                $return = ($attempt == 1)? $this->getDefaultPartId($messageId, 'text/plain', 2) : false;
                break;
            }
            case 'text/plain':
            {
                $return = ($attempt == 1)? $this->getDefaultPartId($messageId, 'text/html', 2) : false;
                break;
            }
            default:
            {
        ...
        if (!$return && $attempt == 2)
        {
            if (isset($this->structure[$messageId]['ftype'][0]))
            {
                $this->structure[$messageId]['fallback'][0] = true;
            }
            else
            {
                $this->warning('Message contains no MIME types.', __FILE__, __LINE__);
            }
        }

        $this->msg[$messageId]['pid'] = !$return? 1 : $return;
    ...
    * @access public
    */
    public function extractMIME(&$messageId, $MIMEs)
    {
        $this->checkIfParsed($messageId);

        $return = array();

        if (is_array($this->structure[$messageId]['ftype']))
        {
            if (is_array($MIMEs))
            {
                foreach ($MIMEs as $MIME)
                {
                    if (0 !== count($keys = array_keys($this->structure[$messageId]['ftype'], $MIME)))
                    {
                        foreach ($keys as $key)
                        {
                            $return[] = $this->structure[$messageId]['pid'][$key];
                        }
                    }
                }
            }
            ...
            {
                if (0 !== count($keys = array_keys($this->structure[$messageId]['ftype'], $MIMEs)))
                {
                    foreach ($keys as $key)
                    {
                        $return[] = $this->structure[$messageId]['pid'][$key];
        ...
        {
            $this->warning('Member variable $this->structure[\'ftype\'] is not an array.', __FILE__, __LINE__);
        }
    ...
    * @see getHeaders
    */
    public function getRawHeaders(&$messageId, $partId = '0', $return = true, $partId_check = false)
    {
        $this->checkIfParsed($messageId);
        ...
        if ($partId !== '0')
        {
            if (false === ($partId = $this->_defaultHeaderPid($messageId, $partId)))
            {
                $this->warning('pid, '.$partId.', is not valid.', __FILE__, __LINE__);
                return false;
            }
        }

        if ($partId === '0' && $partId_check)
        {
            return true;
        }
        else if ($partId_check)
        {
            $return = true;
        }
        ...
        if ($partId === '0')
        {
            ...
            $rawHeaders = imap_fetchheader($this->mailbox, $messageId, $opt);
        }
        else
        {
            $opt = (isset($this->option['fetchbody']))? $this->option['fetchbody'] : null;
            $rawHeaders = imap_fetchbody($this->mailbox, $messageId, $partId, $opt);
        }
        ...
        {
            $this->header[$messageId]['raw'] = $rawHeaders;
            return true;
        }
    }
    ...
    {
        $this->hMailIMAPFromLength = (int) $length;
    }
    
    public function setSubjectLength($length)
    {
        $this->hMailIMAPSubjectLength = (int) $length;
    }
    
    public function setDefaultHost($host)
    {
        $this->hMailIMAPDefaultHost = $host;
    }
    ...
    */
    public function getHeaders(&$messageId, $partId = '0', $return = true)
    {
        $this->checkIfParsed($messageId);
        ...
        if ($partId !== '0')
        {
            if (false === ($rawHeaders = $this->getRawHeaders($messageId, $partId, true, true)))
            {
                return false;
            }
        ...
        if ($partId === '0')
        {
            $headerInformation = imap_headerinfo(
                $this->mailbox,
                $messageId,
                $this->hMailIMAPFromLength(1024),
                $this->hMailIMAPSubjectLength(1024),
                $this->hMailIMAPDefaultHost($_SERVER['HTTP_HOST'])
            );
        }
        else
        {
            $headerInformation = imap_rfc822_parse_headers(
                $rawHeaders,
                $this->hMailIMAPDefaultHost($_SERVER['HTTP_HOST'])
            );
        }
        ...
        if (!is_object($headerInformation))
        {
            $this->warning('pid, '.$partId.', in not valid.', __FILE__, __LINE__);
            return false;
        }

        $headers = get_object_vars($headerInformation);

        foreach ($headers as $key => $value)
        {
            if (!is_object($value) && !is_array($value))
            {
                $this->header[$messageId][$key] = iconv_mime_decode($value, 2, 'UTF-8');
            }
        }

        // copy udate or create it from date string.
        if (isset($headerInformation->udate) && !empty($headerInformation->udate))
        {
            $this->header[$messageId]['udate'] = (int) $headerInformation->udate;
        }
        else
        {
            strtotime($headerInformation->Date);
        }
        ...
        {
            if (isset($headerInformation->$line[$i]))
            {
                $this->parseHeaderLine($messageId, $headerInformation->$line[$i], $line[$i]);
            }
        }

        // All possible information has been copied, destroy original object
        unset($headerInformation);
        return ($return)? $this->header[$messageId] : false;
    }
    ...
    */
    private function parseHeaderLine(&$messageId, &$line, $name)
    {
        if (isset($line) && count($line) >= 1)
        {
            ...
            foreach ($line as $object)
            {
                if (isset($object->adl))
                {
                    $this->header[$messageId][$name.'_adl'][$i] = iconv_mime_decode($object->adl, 2, 'UTF-8');
                }
                
                if (isset($object->mailbox))
                {
                    $this->header[$messageId][$name.'_mailbox'][$i] = iconv_mime_decode($object->mailbox, 2, 'UTF-8');
                }
                
                if (isset($object->personal))
                {
                    $this->header[$messageId][$name.'_personal'][$i] = iconv_mime_decode($object->personal, 2, 'UTF-8');
                }
        
                if (isset($object->host))
                {
                    $this->header[$messageId][$name.'_host'][$i] = iconv_mime_decode($object->host, 2, 'UTF-8');
                }
                
                if (isset($object->mailbox) && isset($object->host))
                {
                    $this->header[$messageId][$name][$i] = iconv_mime_decode($object->mailbox, 2, 'UTF-8').'@'.iconv_mime_decode($object->host, 2, 'UTF-8');
                }
            ...
            {
                $this->header[$messageId][$name.'address'][$i] = ${$name.'address'};
            }
        }
    }
    ...
    */
    private function defaultHeaderPid(&$messageId, $partId)
    {
        ...
        if (false !== ($i = array_search((string) $partId, $this->structure[$messageId]['pid'])))
        {
            ...
            if ($this->structure[$messageId]['ftype'][$i] == 'message/rfc822')
            {
                $return = (string) $partId.'.0';
            }
            else if ($partId == $this->msg[$messageId]['pid'])
            {
                $return = (string) '0';
            }
            else
            {
                $partIdLength = strlen($partId);
                $this_nesting = count(explode('.', $partId));

                // Deeper searching may be required, go back to this part's parent.
                if (!stristr($partId, '.') || ($this_nesting - 1) == 1)
                {
                    $return = (string) '0';
                }
                else if ($this_nesting > 2)
                {
                    ...
                    {
                        foreach ($this->structure[$messageId]['pid'] as $p => $aid)
                        {
                            $nesting = count(explode('.', $this->structure[$messageId]['pid'][$p]));
        
                            if ($nesting == $pos && ($this->structure[$messageId]['ftype'][$p] == 'message/rfc822' || $this->structure[$messageId]['ftype'][$p] == 'multipart/related'))
                            {
                                // Break iteration and return!
                                return (string) $this->structure[$messageId]['pid'][$p].'.0';
                            }
                        }
                    }
        
                    $return = ($partIdLength == 3)? (string) '0' : false;
                }
                else
                {
                    $return = false;
    ...
    * @see getHeaders
    */
    public function unsetHeaders(&$messageId)
    {
        unset($this->header[$messageId]);
        return;
    }
    ...
    */
    public function delete(&$messageId, $expunge = true)
    {
        if (!is_array($messageId))
        {
            ...
            {
                $this->warning('Unable to mark message for deletion.', __FILE__, __LINE__);
                $return = false;
            }
            else
            {
    ...
    */
    public function expunge()
    {
        ...
        {
            $this->warning('Unable to expunge mailbox.', __FILE__, __LINE__);
            return false;
        }
    }
    ...
    */
    public function getErrors($handler = true, $seperator = "\n")
    {
        ...
        if ($handler)
        {
            $this->warning(implode($seperator, $errors), __FILE__, __LINE__);
        }
    ...
    */
    public function getAlerts($handler = true, $seperator = "\n")
    {
        ...
        if ($handler)
        {
            $this->warning(implode($seperator, $alerts), __FILE__, __LINE__);
        }
    ...
    */
    public function getQuota($folder = null, $return = true)
    {
        ...
        {
            $this->warning('Quota not available for this server.', __FILE__, __LINE__);
            return false;
        }
        else if ($return)
        {
    ...
    */
    public function setFlags(array $messageIds, array $flags)
    {        
        return imap_setflag_full(
            $this->mailbox, 
            implode(',', $messageIds), 
            implode(' ', $flags), 
            $this->hMailIMAPSetFlagOptions(null)
        );
    }
    
    public function clearFlags(array $messageIds, array $flags)
    {
        return imap_clearflag_full(
            $this->mailbox,
            implode(',', $messageIds),
            implode(' ', $flags),
            $this->hMailIMAPClearFlagOptions(null)
        );
    }
    ...
    */
    public function getMailboxes($host = null, $pattern = '*', $return = true)
    {
        if (empty($host) && !isset($this->mailboxInfo['host']))
        {
            $this->warning('Supplied host is not valid!');
            return false;
        }
        ...
        {
            $this->warning('Cannot fetch mailbox names.', __FILE__, __LINE__);
            return false;
        }
