$.fn.extend({
    ...
    editorClickEvents : function(event)
    {
/*
        var node = '';
...
var editor = {
    ...
    ready : function()
    {
        if (top)
        {
            // Copies editor jQuery plugins to the top frame, so that those files don't have
            // to be reincluded there.
            for (var plugin in $.fn)
            {
                if (typeof $.fn[plugin] == 'function' && typeof top.$.fn[plugin] == 'undefined')
                {
                    top.$.fn[plugin] = $.fn[plugin];
                }
            }
        }
        ...
        {
            $('div#hEditorTemplateBodyWrapper').addClass('hEditorTemplateIsEmbedded');
        }
        ...
        if (this.documentSelector)
        {
            ...
            {
                $(this.documentContainerSelector).wrap(
                    $('<div/>').attr('id', 'hEditorTemplateDocumentWrapper')
                );
            }
            else
            {
        ...
        if (this.titleSelector)
        {
            ...
            this.title
                ...
                .wrap(
                    $('<div/>').attr('id', 'hEditorTemplateTitleWrapper')
                );
        }
    }
};
