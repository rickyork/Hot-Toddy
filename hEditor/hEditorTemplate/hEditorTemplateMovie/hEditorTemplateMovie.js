...
editor.movie = {
    ...
    removeMovie : function()
    {
        if (this.video && this.video.length)
        {
            ...
            {
                this.video.parents('div.hEditorTemplateNodeWrapper').remove();
            }
            else
            {
                this.video.remove();
    ...
    openDialogue : function()
    {
        ...
        if (video.length)
        {
            $('input#hEditorMovie').val(video.attr('src'));
        }

        if (this.video && this.video.length)
        {
            $('div#hEditorTemplateMovieRemoveWrapper').removeClass('hEditorTemplateDisabledButtons');
        }

        editor.openModal();
        $('div#hEditorTemplateMovie').slideDown('slow');
        $('li#hEditorTemplateMovie').commandOn();
    },

    closeDialogue : function()
    {
        editor.closeModal();
        $('div#hEditorTemplateMovie').slideUp('slow');
        $('li#hEditorTemplateMovie').commandOff();

        this.video = null;

        $('div#hEditorTemplateMovieRemoveWrapper').addClass('hEditorTemplateDisabledButtons');

        $('input#hEditorMovie').val('');
        $('input#hEditorMovieTarget').removeAttr('checked');
    }
};
