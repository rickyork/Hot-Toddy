...
class hGoogleSiteMap extends hPlugin {
    ...
    public function hConstructor()
    {
        ...
        {
            $variables = $this->getFileVariables($data['hFileId']);

            $hFiles['hFilePath'][]         = $data['hFilePath'];
            $hFiles['hFileId'][]           = $data['hFileId'];
            $hFiles['hFileLastModified'][] = date('Y-m-d', empty($data['hFileLastModified'])? $data['hFileCreated'] : $data['hFileLastModified']);

            if (isset($variables['hGoogleChangeFrequency']))
            {
                $hFiles['hGoogleChangeFrequency'][] = $variables['hGoogleChangeFrequency'];
            }

            if (isset($variables['hGooglePriority']))
            {
                $hFiles['hGooglePriority'][] = $variables['hGooglePriority'];
            }
        }
