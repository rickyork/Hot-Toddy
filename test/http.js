...
var onStateChange = function()
{
    ...
    switch (readyState)
    {
        case 4:
        {
            var mime = this.http.getResponseHeader('Content-Type').toLowerCase();
            var data = '';
            ...
            switch (true)
            {
                ...
                {
                    data = $.parseJSON(this.http.responseText);
                    break;
                };
            }
            ...
            {
                this.fn.call(this.context? this.context : data, data, obj);
            }
...
var $http = hot.factory();

$http.prototype.init = function(type, url, data, fn, context)
{
    this.url = url;
    this.type = type;
    this.data = hot.getQueryString(data);
    this.fn = fn;
    ...
    if (this.http)
    {
        ...
        this.http.open(this.type, this.url, true);
...
var http = new function() {
    ...
    var request = function(type, url, data, fn, context, synchronous)
    {
        if (!synchronous)
        {
            new $http(type, url, data, fn, context, synchronous);
        }
        else
        {
            xhttp = xmlhttp();
            ...
            xhttp.send(hot.getQueryString(data));

            return onStateChange.call({
                    http : xhttp,
                    type : type,
                    url : url,
                    data : data,
